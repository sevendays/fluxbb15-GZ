...
function gz_ak_first_time()
{
	...
	while ($table = $db->fetch_row($result))
		$tables[] = $table[0];
	if(in_array($db->prefix.'gz_akismet', $tables)) return false;
	return true;
}
...
function gz_ak_install()
{
	...
	switch ($db_type)
	{
		case 'mysql':
		case 'mysqli':
			$sql = 'CREATE TABLE '.$db->prefix."gz_akismet (
	...
	$db->query($sql) or error('Unable to create table '.$db->prefix.'gz_akismet. Please check your settings and try again.',  __FILE__, __LINE__, $db->error());	
					...
					<fieldset>
						...
						<div class="infldset">
							...
							<table class="aligntop" cellspacing="0">
								<tr>
									<th scope="row">
										<?php echo $gz_ak_lang['database create table'] . ' gz_akismet'; ?>
									</th>
									<td>
										<div>
